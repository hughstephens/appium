...
androidCommon.pushUnicodeIME = function (cb) {
  ...
  var imePath = path.resolve(__dirname, "..", "..", "..", "build",
      "utf7ime", "utf7ime-debug.apk");
  fs.stat(imePath, function (err) {
    if (err) {
      cb(new Error("Could not find utf7 ime apk; please run " +
...
androidCommon.packageAndLaunchActivityFromManifest = function (cb) {
  ...
  this.adb.packageAndLaunchActivityFromManifest(this.args.app, function (err, pkg, act) {
    ...
    if (act === null) {
      var appiumApkToolsJarPath = ADB.jars['appium_apk_tools.jar'];
  ...
  if (this.args.unicodeKeyboard) {
    ...
    this.pushUnicodeIME(function (err) {
      ...
      this.adb.defaultIME(function (err, engine) {
        ...
        logger.debug('Setting IME to \'jp.jun_nama.test.utf7ime/.Utf7ImeService\'');
        this.adb.enableIME('jp.jun_nama.test.utf7ime/.Utf7ImeService', function (err) {
          if (err) return cb(err);
          this.adb.setIME('jp.jun_nama.test.utf7ime/.Utf7ImeService', cb);
        }.bind(this));
      }.bind(this));
    }.bind(this));
  } else {
